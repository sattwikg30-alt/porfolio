...
export default function SalvusFlow(){
  ...
  const CX=120,R=20,STEP=50,START=40;
    ...
    <svg viewBox={`0 0 500 ${lastY+R+24}`} style={{width:"100%",height:"100%",display:"block"}} fill="none"
      ...
      {NODES.map((n,i)=>{const y=ys[i],h=hov===n.id,last=i===NODES.length-1;return(
        <g key={n.id} onMouseEnter={()=>setHov(n.id)} onMouseLeave={()=>setHov(null)} style={{cursor:"default"}}>
          <circle cx={CX} cy={y} r={h?R+10:R+3} fill="none" stroke={h?`rgba(${RGB},0.4)`:`rgba(${RGB},0.07)`} strokeWidth="1.5" style={{transition:"all 0.3s ease"}}/>
          ...
          <text x={CX} y={y+5} textAnchor="middle" fill={h?COLOR:`rgba(${RGB},0.6)`} style={{fontSize:14,transition:"fill 0.3s ease"}}>{n.icon}</text>
          <line x1={CX+R} y1={y} x2={CX+R+16} y2={y} stroke={h?`rgba(${RGB},0.6)`:`rgba(${RGB},0.18)`} strokeWidth="1.5" style={{transition:"stroke 0.3s ease"}}/>
          <text x={CX+R+24} y={y-6} fill={h?"#f0e8ff":"#c4c4d8"} style={{fontSize:h?13:12,fontFamily:"monospace",fontWeight:"bold",letterSpacing:"0.11em",transition:"all 0.3s ease"}}>{n.label}</text>
          <text x={CX+R+24} y={y+8} fill={`rgba(${RGB},0.72)`} style={{fontSize:10,fontFamily:"monospace",opacity:h?1:0.35,transition:"opacity 0.3s ease"}}>{n.sub}</text>
          {i<NODES.length-1&&<text x={CX-5} y={(y+ys[i+1])/2+4} fill={`rgba(${RGB},0.28)`} style={{fontSize:12}}>↓</text>}
        </g>);})}
    </svg>);
}
