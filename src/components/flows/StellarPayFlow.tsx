...
const STEPS=[
  {id:"wallet", label:"WALLET",         sub:"Freighter wallet connection",       y:40 },
  {id:"balance",label:"BALANCE",        sub:"Live XLM balance sync",             y:90},
  {id:"send",   label:"SEND XLM",       sub:"Native Stellar transfer",           y:140},
  {id:"sign",   label:"SIGN",           sub:"Wallet confirmation prompt",        y:190},
  {id:"testnet",label:"STELLAR TESTNET",sub:"Transaction broadcast",             y:240},
  {id:"ledger", label:"LEDGER",         sub:"Block confirmation & metadata",     y:290},
  {id:"history",label:"TX HISTORY",     sub:"Sent / received filter view",       y:340},
];
export default function StellarPayFlow(){
  ...
  const CX=120,R=20,firstY=STEPS[0].y,lastY=STEPS[STEPS.length-1].y;
    ...
    <svg viewBox={`0 0 500 ${lastY+R+24}`} style={{width:"100%",height:"100%",display:"block"}} fill="none"
      ...
      {STEPS.map((s,i)=>{const h=hov===s.id,last=i===STEPS.length-1,confirm=s.id==="sign"||s.id==="ledger";return(
        <g key={s.id} onMouseEnter={()=>setHov(s.id)} onMouseLeave={()=>setHov(null)} style={{cursor:"default"}}>
          <circle cx={CX} cy={s.y} r={h?R+10:R+3} fill="none" stroke={h?`rgba(${RGB},0.42)`:`rgba(${RGB},0.07)`} strokeWidth="1.5" style={{transition:"all 0.3s ease"}}/>
          ...
          {confirm&&<text x={CX} y={s.y+5} textAnchor="middle" fill={h?COLOR:`rgba(${RGB},0.5)`} style={{fontSize:14,transition:"fill 0.3s ease"}}>✓</text>}
          ...
          <text x={CX+R+24} y={s.y-6} fill={h?"#dde0ff":"#c4c4d8"} style={{fontSize:h?13:12,fontFamily:"monospace",fontWeight:"bold",letterSpacing:"0.11em",transition:"all 0.3s ease"}}>{s.label}</text>
          <text x={CX+R+24} y={s.y+8} fill={`rgba(${RGB},0.72)`} style={{fontSize:10,fontFamily:"monospace",opacity:h?1:0.35,transition:"opacity 0.3s ease"}}>{s.sub}</text>
          {i<STEPS.length-1&&<text x={CX-5} y={(s.y+STEPS[i+1].y)/2+4} fill={`rgba(${RGB},0.28)`} style={{fontSize:12}}>↓</text>}
        </g>);})}
    </svg>);
}
