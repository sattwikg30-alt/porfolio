...
const NODES=[
  {id:"campaign",label:"CAMPAIGN",       sub:"Created by project owner",       y:48 },
  {id:"stellar", label:"STELLAR/SOROBAN",sub:"Decentralised Testnet infra",     y:128},
  {id:"contract",label:"SMART CONTRACT", sub:"Enforces campaign rules",         y:208},
];
const WALLETS=[{x:85,label:"FREIGHTER"},{x:215,label:"ALBEDO"},{x:345,label:"xBULL"}];
const LOWER=[
  {id:"xlm",     label:"XLM FUNDING",     sub:"Native Stellar Lumens contributions", y:318},
  {id:"activity",label:"PUBLIC AUDIT",    sub:"Live transparent activity feed",      y:408},
];
export default function StellarFundFlow(){
  ...
  const CX=215,R=24,RW=22,WALLET_Y=265,firstY=NODES[0].y,lastY=LOWER[LOWER.length-1].y;
    ...
    <svg viewBox={`0 0 430 ${totalH}`} style={{width:"100%",height:"100%",display:"block"}} fill="none"
      ...
      {NODES.map((n,i)=>{const h=hov===n.id;const nxt=NODES[i+1];return(
        <g key={n.id} onMouseEnter={()=>setHov(n.id)} onMouseLeave={()=>setHov(null)} style={{cursor:"default"}}>
          <circle cx={CX} cy={n.y} r={h?R+11:R+3} fill="none" stroke={h?`rgba(${RGB},0.4)`:`rgba(${RGB},0.07)`} strokeWidth="1.5" style={{transition:"all 0.3s ease"}}/>
          ...
          <text x={CX+R+22} y={n.y-6} fill={h?"#c0e8ff":"#c4c4d8"} style={{fontSize:h?13:11,fontFamily:"monospace",fontWeight:"bold",letterSpacing:"0.1em",transition:"all 0.3s ease"}}>{n.label}</text>
          <text x={CX+R+22} y={n.y+9} fill={`rgba(${RGB},0.72)`} style={{fontSize:10,fontFamily:"monospace",opacity:h?1:0.35,transition:"opacity 0.3s ease"}}>{n.sub}</text>
          {nxt&&<text x={CX-6} y={(n.y+nxt.y)/2+5} fill={`rgba(${RGB},0.28)`} style={{fontSize:13}}>↓</text>}
        </g>);})}
      ...
      {LOWER.map((n,i)=>{const h=hov===n.id,last=i===LOWER.length-1;return(
        <g key={n.id} onMouseEnter={()=>setHov(n.id)} onMouseLeave={()=>setHov(null)} style={{cursor:"default"}}>
          ...
          <text x={CX+R+22} y={n.y-6} fill={h?"#c0e8ff":"#c4c4d8"} style={{fontSize:h?13:11,fontFamily:"monospace",fontWeight:"bold",letterSpacing:"0.1em",transition:"all 0.3s ease"}}>{n.label}</text>
          <text x={CX+R+22} y={n.y+9} fill={`rgba(${RGB},0.72)`} style={{fontSize:10,fontFamily:"monospace",opacity:h?1:0.35,transition:"opacity 0.3s ease"}}>{n.sub}</text>
          {i===0&&<text x={CX-6} y={(n.y+LOWER[1].y)/2+5} fill={`rgba(${RGB},0.28)`} style={{fontSize:13}}>↓</text>}
        </g>);})}
    </svg>);
}
